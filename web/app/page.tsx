...
function formToRequirement(f: Any): Any {
  const req: Any = { name: f.name };
  ...
  if (f.comms && f.comms.trim()) req.comms = f.comms.split(",").map((s: string) => s.trim()).filter(Boolean);
...
function requirementToForm(req: Any = {}): Any {
  const f: Any = { ...BLANK_FORM, name: req.name || "" };
  const s = (k: string, v: Any) => { if (v !== undefined && v !== null) f[k] = v; };
...
function field(c: Any, label: string, val: Any, unit = "") {
...
const ACT_STYLE: Record<string, { c: string; g: string }> = {
...
export default function App() {
  const [form, setForm] = useState<Any>(PRESETS.drone);
  ...
  const [bench, setBench] = useState<Any>(null); // {model: {coverage, iterations, done}}
  const [showAbout, setShowAbout] = useState(false);
  const [inputMode, setInputMode] = useState("describe"); // describe | manual
  const [chat, setChat] = useState<Any[]>([]);     // {role, content}
  ...
  const actEndRef = useRef<HTMLDivElement | null>(null);
  ...
  function applyPreset(k: string) { setForm(PRESETS[k]); }

  function applySpec(spec: Any) {
    if (!spec) return;
    const byId = Object.fromEntries(spec.checks.map((c: Any) => [c.constraint_id, c]));
  ...
  function handleEvent(ev: Any) {
    if (ev.type === "rubric") {
      setRubric(ev.rubric.map((c: Any) => ({ ...c, status: "pending" })));
    ...
    try {
      await streamSSE("/api/bench", body, (ev) => {
        if (ev.type === "done" && ev.model) {
          setBench((b: Any) => ({ ...b, [ev.model]: { coverage: ev.coverage, iterations: ev.iterations, done: true } }));
        }
  ...
  const subsystems: Any[] = bom ? Object.entries(bom.subsystems).filter(([, v]: Any) => v.length) : [];
  ...
  return (
    <div className="min-h-full p-4 lg:p-6 max-w-[1500px] mx-auto">
      ...
      {bench && (
        <div className="surface px-5 py-3 mb-4">
          ...
          <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${Object.keys(bench).length}, minmax(0,1fr))` }}>
            {Object.entries(bench).map(([m, r]: Any) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr_1fr] gap-4">
        ...
        {/* RIGHT: BOM tree */}
        <section className="surface p-4">
          <SectionTitle n="2" t="Bill of materials" sub={subsystems.length ? `${subsystems.reduce((n, [, v]: Any) => n + v.length, 0)} parts` : ""} />
...
function Inp({ label, v, on, full }: { label: string; v: Any; on: (v: string) => void; full?: boolean }) {
  return (
